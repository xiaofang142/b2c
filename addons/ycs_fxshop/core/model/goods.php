...
class Ycs_DShop_Goods
{
    public function getOption($goodsid = 0, $optionid = 0)
    {
        global $_W;
        return pdo_fetch('select * from ' . tablename('ycs_fxshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid', array(
            ':id' => $optionid,
            ':uniacid' => $_W['uniacid'],
            ':goodsid' => $goodsid
        ));
    ...
    public function getList($args = array())
    {
        ...
        if (!$random) {
            $sql = "SELECT id,title,thumb,marketprice,productprice,sales,total FROM " . tablename('ycs_fxshop_goods') . " where 1 {$condition} ORDER BY {$order} {$orderby} LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;
        } else {
            $sql = "SELECT id,title,thumb,marketprice,productprice,sales,total FROM " . tablename('ycs_fxshop_goods') . " where 1 {$condition} ORDER BY rand() LIMIT " . $pagesize;
        }
    ...
    public function getComments($goodsid = '0', $args = array())
    {
        ...
        $sql       = "SELECT id,nickname,headimgurl,content,images FROM " . tablename('ycs_fxshop_goods_comment') . " where 1 {$condition} ORDER BY createtime desc LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;
        $list      = pdo_fetchall($sql, $params);
        foreach ($list as &$row) {
