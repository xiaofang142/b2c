...
class Ycs_DShop_Notice
{
    public function sendOrderMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('ycs_fxshop_order') . ' where id=:id limit 1', array(
            ':id' => $orderid
        ));
        if (empty($order)) {
            return;
        }
        $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ycs_fxshop&do=order&p=detail&id=' . $orderid;
        ...
        $openid      = $order['openid'];
        $order_goods = pdo_fetchall('select g.id,g.title,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join ' . tablename('ycs_fxshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $goods       = '';
        ...
        if ($order['status'] == -1) {
            if (empty($order['dispatchtype'])) {
                $address      = pdo_fetch('select * from ' . tablename('ycs_fxshop_member_address') . ' where id=:id and uniacid=:uniacid limit 1 ', array(
                    ":uniacid" => $_W['uniacid'],
                    ":id" => $order['addressid']
                ));
                $orderAddress = array(
            ...
            if (strexists($pay_detailurl, '/addons/ycs_fxshop/')) {
                $pay_detailurl = str_replace("/addons/ycs_fxshop/", '/', $pay_detailurl);
            }
    ...
    public function sendMemberUpgradeMessage($openid = '', $oldlevel = null, $level = null)
    {
        ...
        $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ycs_fxshop&do=member';
        if (strexists($detailurl, '/addons/ycs_fxshop/')) {
            $detailurl = str_replace("/addons/ycs_fxshop/", '/', $detailurl);
        }
        if (strexists($detailurl, '/core/mobile/order/')) {
    ...
    {
        global $_W, $_GPC;
        $log_info   = pdo_fetch('select * from ' . tablename('ycs_fxshop_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
            ':id' => $log_id,
            ':uniacid' => $_W['uniacid']
        ));
        ...
        if ($log_info['type'] == 0) {
            ...
            $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ycs_fxshop&do=member&p=log&type=1';
            if (strexists($detailurl, '/addons/ycs_fxshop/')) {
                $detailurl = str_replace("/addons/ycs_fxshop/", '/', $detailurl);
            }
