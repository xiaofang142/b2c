...
class Ycs_DShop_Common
{
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
    {
        ...
        $set['sign_type']      = 'MD5';
        if (empty($type)) {
            $set['notify_url'] = $_W['siteroot'] . "addons/ycs_fxshop/payment/alipay/notify.php";
            $set['return_url'] = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=ycs_fxshop&do=order&p=pay&op=return&openid=" . $openid;
        } else {
            $set['notify_url'] = $_W['siteroot'] . "addons/ycs_fxshop/payment/alipay/notify.php";
            $set['return_url'] = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=ycs_fxshop&do=member&p=recharge&op=return&openid=" . $openid;
        }
    ...
    function wechat_build($params, $wechat, $type = 0)
    {
        ...
        if ($wechat['version'] == 1) {
            ...
            $package['device_info']      = "ycs_fxshop";
            $package['out_trade_no']     = $params['tid'];
            $package['total_fee']        = $params['fee'] * 100;
            $package['fee_type']         = '1';
            $package['notify_url']       = $_W['siteroot'] . "addons/ycs_fxshop/payment/wechat/notify.php";
    ...
    public function createNO($table, $field, $prefix)
    {
        ...
        while (1) {
            $count = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_' . $table) . " where {$field}=:billno limit 1", array(
                ':billno' => $billno
            ));
            if ($count <= 0) {
                break;
    ...
    public function getSec($uniacid = 0)
    {
        ...
        $set = pdo_fetch("select sec from " . tablename('ycs_fxshop_sysset') . ' where uniacid=:uniacid limit 1', array(
            ':uniacid' => $uniacid
        ));
        if (empty($set)) {
            $set = array();
