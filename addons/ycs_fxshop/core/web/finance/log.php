...
if ($op == 'display') {
    ...
    $total = pdo_fetchcolumn("select count(*) from " . tablename('ycs_fxshop_member_log') . " log " . " left join " . tablename('ycs_fxshop_member') . " m on m.openid=log.openid and m.uniacid= log.uniacid" . " left join " . tablename('ycs_fxshop_member_group') . " g on m.groupid=g.id" . " left join " . tablename('ycs_fxshop_member_level') . " l on m.level =l.id" . " where 1 {$condition} ", $params);
    ...
    $log     = pdo_fetch('select * from ' . tablename('ycs_fxshop_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $id,
        ':uniacid' => $uniacid
    ));
    if (empty($log)) {
