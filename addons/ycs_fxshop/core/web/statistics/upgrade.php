...
if ($op == 'display') {
	...
	if ($op == 'check') {
		...
		foreach ($my_scenfiles as $sf) {
			$files[] = array('path' => str_replace(IA_ROOT . '/addons/ycs_fxshop/', "", $sf), 'md5' => md5_file($sf));
		}
		$files = base64_encode(json_encode($files));
		$version = defined('YCS_FXSHOP_VERSION') ? YCS_FXSHOP_VERSION : '1.0';
		$resp = ihttp_post(YCS_FXSHOP_AUTH_URL, array('type' => 'check', 'ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => $auth['domain'], 'version' => $version, 'files' => $files));
		...
		if (is_array($ret)) {
			if ($ret['result'] == 1) {
				...
				$tmpdir = IA_ROOT . '/addons/ycs_fxshop/' . date('ymd');
				if (!is_dir($tmpdir)) {
					mkdirs($tmpdir);
				}
		...
		if ($op == 'download') {
			...
			if (!empty($path)) {
				$resp = ihttp_post(YCS_FXSHOP_AUTH_URL, array('type' => 'download', 'ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => $auth['domain'], 'path' => $path));
				$ret = @json_decode($resp['content'], true);
				if (is_array($ret)) {
					$path = $ret['path'];
