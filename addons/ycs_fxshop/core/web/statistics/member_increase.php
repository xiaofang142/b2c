...
if (!empty($days)) {
    ...
    for ($i = $days; $i >= 0; $i--) {
        ...
        $datas[]   = array(
            'date' => $time,
            'mcount' => pdo_fetchcolumn("select count(*) from " . tablename('ycs_fxshop_member') . " where isagent=0  {$condition}", $params),
            'acount' => pdo_fetchcolumn("select count(*) from " . tablename('ycs_fxshop_member') . " where isagent=1  {$condition}", $params)
        );
    }
