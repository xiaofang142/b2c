...
{
    global $_W;
    $path = IA_ROOT . "/addons/ycs_fxshop/cert";
    load()->func('file');
    mkdirs($path, '0777');
...
if ($op == 'template') {
    $styles = array();
    $dir    = IA_ROOT . "/addons/ycs_fxshop/template/mobile/";
    ...
    if (isset($set['notice']['openid'])) {
        if (!empty($set['notice']['openid'])) {
            ...
            $salers = pdo_fetchall("select id,nickname,avatar,openid from " . tablename('ycs_fxshop_member') . ' where openid in (' . implode(",", $openids) . ") and uniacid={$_W['uniacid']}");
        }
    }
...
if (checksubmit()) {
    if ($op == 'shop') {
        ...
        if (!$_W['isfounder']) {
            ...
            file_put_contents(IA_ROOT . '/addons/ycs_fxshop/data/receive_time', $set['trade']['receivetime']);
        }
    ...
    if (empty($setdata)) {
        pdo_insert('ycs_fxshop_sysset', $data);
    } else {
        pdo_update('ycs_fxshop_sysset', $data, array(
            'uniacid' => $_W['uniacid']
        ));
    }
    $setdata   = pdo_fetch("select * from " . tablename('ycs_fxshop_sysset') . ' where uniacid=:uniacid limit 1', array(
