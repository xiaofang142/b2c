...
if ($operation == 'addtype') {
    ...
    $list = pdo_fetchall('SELECT * FROM ' . tablename('ycs_fxshop_member_message_template') . ' WHERE uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
} elseif ($operation == 'post') {
    if (!empty($_GPC['id'])) {
        $list = pdo_fetch('SELECT * FROM ' . tablename('ycs_fxshop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array('id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));
    ...
    if ($_W['ispost']) {
        ...
        if (empty($id)) {
            ...
            pdo_update('ycs_fxshop_member_message_template', $insert, array('id' => $id));
        }
    ...
    $id = intval($_GPC['id']);
    pdo_delete('ycs_fxshop_member_message_template', array('id' => $id, 'uniacid' => $_W['uniacid']));
    message('删除成功！', $this->createWebUrl('member/message'), 'success');
} elseif ($operation == 'send') {
    $id = intval($_GPC['id']);
    $send = pdo_fetch('SELECT * FROM ' . tablename('ycs_fxshop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array('id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($send)) {
    ...
    $value1 = $_GPC['value1'];
    $tpid1 = $_GPC['tpid'];
    pdo_update('ycs_fxshop_member_message_template', array('sendtimes' => $send['sendtimes'] + 1), array('id' => $id));
    if ($class1 == 1) {
    ...
    $id = intval($_GPC['id']);
    $template = pdo_fetch('SELECT * FROM ' . tablename('ycs_fxshop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array('id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($template)) {
    ...
    pdo_update('ycs_fxshop_member_message_template', array('sendcount' => $template['sendcount'] + 1), array('id' => $id));
