...
if ($op == 'display') {
    ...
    $list = pdo_fetchall('select log.id,m.realname,m.avatar,m.weixin,log.type,log.status,log.rechargetype,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname from ' . tablename('ycs_fxshop_member_log') . ' log ' . ' left join ' . tablename('ycs_fxshop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('ycs_fxshop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ycs_fxshop_member_level') . ' l on m.level =l.id' . " where 1 {$condition} ORDER BY log.createtime DESC limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
    if ($_GPC['export'] == 1) {
        foreach ($list as &$row) {
    ...
    $total = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_member_log') . ' log ' . ' left join ' . tablename('ycs_fxshop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('ycs_fxshop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ycs_fxshop_member_level') . ' l on m.level =l.id' . " where 1 {$condition} ", $params);
    ...
    if ($op == 'pay') {
        ...
        $log = pdo_fetch('select * from ' . tablename('ycs_fxshop_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $uniacid));
        if (empty($log)) {
            message('未找到记录!', '', 'error');
        }
        if ($paytype == 'manual') {
            pdo_update('ycs_fxshop_member_log', array('status' => 1), array('id' => $id, 'uniacid' => $uniacid));
