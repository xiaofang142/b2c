...
$sql      = 'SELECT * FROM ' . tablename('ycs_fxshop_category') . ' WHERE `uniacid` = :uniacid ORDER BY `parentid`, `displayorder` DESC';
...
if ($operation == 'post') {
    ...
    if (!empty($id)) {
        $item = pdo_fetch("SELECT * FROM " . tablename('ycs_fxshop_goods') . " WHERE id = :id", array(
            ':id' => $id
        ));
        if (empty($item)) {
        ...
        $allspecs  = pdo_fetchall("select * from " . tablename('ycs_fxshop_goods_spec') . " where goodsid=:id order by displayorder asc", array(
            ":id" => $id
        ));
        foreach ($allspecs as &$s) {
            $s['items'] = pdo_fetchall("select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from " . tablename('ycs_fxshop_goods_spec_item') . " a left join " . tablename('ycs_fxshop_virtual_type') . " b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc", array(
                ":specid" => $s['id']
            ));
        }
        unset($s);
        $params   = pdo_fetchall("select * from " . tablename('ycs_fxshop_goods_param') . " where goodsid=:id order by displayorder asc", array(
            ':id' => $id
        ));
        ...
        $html    = "";
        $options = pdo_fetchall("select * from " . tablename('ycs_fxshop_goods_option') . " where goodsid=:id order by id asc", array(
            ':id' => $id
        ));
        $specs   = array();
        if (count($options) > 0) {
        ...
        if (!empty($item['storeids'])) {
            $stores = pdo_fetchall('select id,storename from ' . tablename('ycs_fxshop_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid']);
        }
    ...
    if (checksubmit('submit')) {
        ...
        if (is_array($_GPC['cates'])) {
            ...
            foreach ($postcates as $pid) {
                if ($cateset['catlevel'] == 3) {
                    $tcate    = pdo_fetch('select id ,parentid from ' . tablename('ycs_fxshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(
                        ':id' => $pid,
                        ':uniacid' => $_W['uniacid']
                    ));
                    $ccate    = pdo_fetch('select id ,parentid from ' . tablename('ycs_fxshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(
                        ':id' => $tcate['parentid'],
                        ':uniacid' => $_W['uniacid']
                    ));
                    $tcates[] = $tcate['id'];
        ...
        if (empty($id)) {
            pdo_insert('ycs_fxshop_goods', $data);
            $id = pdo_insertid();
            plog('shop.goods.add', "添加商品 ID: {$id}");
        } else {
            unset($data['createtime']);
            pdo_update('ycs_fxshop_goods', $data, array(
                'id' => $id
            ));
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (!is_numeric($get_param_id)) {
                pdo_insert("ycs_fxshop_goods_param", $a);
                $param_id = pdo_insertid();
            } else {
                pdo_update('ycs_fxshop_goods_param', $a, array(
                    'id' => $get_param_id
                ));
        ...
        if (count($paramids) > 0) {
            pdo_query("delete from " . tablename('ycs_fxshop_goods_param') . " where goodsid=$id and id not in ( " . implode(',', $paramids) . ")");
        } else {
            pdo_query('delete from ' . tablename('ycs_fxshop_goods_param') . " where goodsid=$id");
        }
        $files       = $_FILES;
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (is_numeric($get_spec_id)) {
                pdo_update("ycs_fxshop_goods_spec", $a, array(
                    "id" => $get_spec_id
                ));
                $spec_id = $get_spec_id;
            } else {
                pdo_insert('ycs_fxshop_goods_spec', $a);
            ...
            for ($n = 0; $n < $itemlen; $n++) {
                ...
                if (is_numeric($get_item_id)) {
                    pdo_update("ycs_fxshop_goods_spec_item", $d, array(
                        "id" => $get_item_id
                    ));
                    $item_id = $get_item_id;
                } else {
                    pdo_insert('ycs_fxshop_goods_spec_item', $d);
            ...
            if (count($itemids) > 0) {
                pdo_query("delete from " . tablename('ycs_fxshop_goods_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id and id not in (" . implode(",", $itemids) . ")");
            } else {
                pdo_query('delete from ' . tablename('ycs_fxshop_goods_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id");
            }
            pdo_update('ycs_fxshop_goods_spec', array(
        ...
        if (count($specids) > 0) {
            pdo_query("delete from " . tablename('ycs_fxshop_goods_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id and id not in (" . implode(",", $specids) . ")");
        } else {
            pdo_query('delete from ' . tablename('ycs_fxshop_goods_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id");
        }
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (empty($get_option_id)) {
                pdo_insert("ycs_fxshop_goods_option", $a);
                $option_id = pdo_insertid();
            } else {
                pdo_update('ycs_fxshop_goods_option', $a, array(
                    'id' => $get_option_id
                ));
        ...
        if (count($optionids) > 0) {
            pdo_query("delete from " . tablename('ycs_fxshop_goods_option') . " where goodsid=$id and id not in ( " . implode(',', $optionids) . ")");
        } else {
            pdo_query('delete from ' . tablename('ycs_fxshop_goods_option') . " where goodsid=$id");
        }
        if ($data['type'] == 3 && $pv) {
    ...
    if ($pv) {
        $virtual_types = pdo_fetchall("select * from " . tablename('ycs_fxshop_virtual_type') . " where uniacid=:uniacid order by id asc", array(
            ":uniacid" => $_W['uniacid']
        ));
    }
    $levels  = m('member')->getLevels();
    $details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename('ycs_fxshop_goods') . " where uniacid=:uniacid and detail_shopname<>''", array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    $sql   = 'SELECT COUNT(*) FROM ' . tablename('ycs_fxshop_goods') . $condition;
    $total = pdo_fetchcolumn($sql, $params);
    if (!empty($total)) {
        $sql   = 'SELECT * FROM ' . tablename('ycs_fxshop_goods') . $condition . ' ORDER BY `status` DESC, `displayorder` DESC,
    ...
    $id  = intval($_GPC['id']);
    $row = pdo_fetch("SELECT id, title, thumb FROM " . tablename('ycs_fxshop_goods') . " WHERE id = :id", array(
        ':id' => $id
    ));
    if (empty($row)) {
