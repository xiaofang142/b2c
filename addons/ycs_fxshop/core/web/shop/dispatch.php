...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('ycs_fxshop_dispatch', array(
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('ycs_fxshop_dispatch') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id asc");
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            ...
            pdo_update('ycs_fxshop_dispatch', $data, array(
                'id' => $id
            ));
        } else {
            pdo_insert('ycs_fxshop_dispatch', $data);
            $id = pdo_insertid();
    ...
    $dispatch = pdo_fetch("SELECT * FROM " . tablename('ycs_fxshop_dispatch') . " WHERE id = '$id' and uniacid = '{$_W['uniacid']}'");
    if (!empty($dispatch)) {
    ...
    $id       = intval($_GPC['id']);
    $dispatch = pdo_fetch("SELECT id,dispatchname FROM " . tablename('ycs_fxshop_dispatch') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
    if (empty($dispatch)) {
        message('抱歉，配送方式不存在或是已经被删除！', $this->createWebUrl('shop/dispatch', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ycs_fxshop_dispatch', array(
        'id' => $id
    ));
