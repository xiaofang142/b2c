...
$children  = array();
$category  = pdo_fetchall("SELECT * FROM " . tablename('ycs_fxshop_category') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY parentid ASC, displayorder DESC");
...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['datas'])) {
        ...
        foreach ($datas as $row) {
            pdo_update("ycs_fxshop_category", array(
                'parentid' => 0,
            ...
            if ($row['children'] && is_array($row['children'])) {
                ...
                foreach ($row['children'] as $child) {
                    pdo_query("update " . tablename('ycs_fxshop_category') . " set  parentid=:parentid,displayorder=:displayorder,level=2 where id=:id", array(
                    ...
                    if ($child['children'] && is_array($child['children'])) {
                        ...
                        foreach ($child['children'] as $third) {
                            pdo_query("update " . tablename('ycs_fxshop_category') . " set  parentid=:parentid,displayorder=:displayorder,level=3 where id=:id", array(
    ...
    if (!empty($id)) {
        ...
        $item     = pdo_fetch("SELECT * FROM " . tablename('ycs_fxshop_category') . " WHERE id = '$id' limit 1");
    ...
    if (!empty($parentid)) {
        $parent = pdo_fetch("SELECT id, parentid, name FROM " . tablename('ycs_fxshop_category') . " WHERE id = '$parentid' limit 1");
        if (empty($parent)) {
            message('抱歉，上级分类不存在或是已经被删除！', $this->createWebUrl('post'), 'error');
        }
        if (!empty($parent['parentid'])) {
            $parent1 = pdo_fetch("SELECT id, name FROM " . tablename('ycs_fxshop_category') . " WHERE id = '{$parent['parentid']}' limit 1");
        }
    }
    if (empty($parent)) {
        $level = 1;
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            unset($data['parentid']);
            pdo_update('ycs_fxshop_category', $data, array(
                'id' => $id
            ));
            load()->func('file');
            ...
            pdo_insert('ycs_fxshop_category', $data);
            $id = pdo_insertid();
    ...
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id, name, parentid FROM " . tablename('ycs_fxshop_category') . " WHERE id = '$id'");
    if (empty($item)) {
        message('抱歉，分类不存在或是已经被删除！', $this->createWebUrl('shop/category', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ycs_fxshop_category', array(
        'id' => $id,
        'parentid' => $id
    ), 'OR');
