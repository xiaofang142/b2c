...
if ($operation == 'display' && $_W['isajax']) {
    ...
    pdo_delete('ycs_fxshop_member_log', array(
        'openid' => $openid,
        'status' => 0,
        'type' => 0,
        'uniacid' => $_W['uniacid']
    ...
    pdo_insert('ycs_fxshop_member_log', $log);
    $logid  = pdo_insertid();
    ...
    $log = pdo_fetch('SELECT * FROM ' . tablename('ycs_fxshop_member_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(
        ':uniacid' => $uniacid,
        ':logno' => $logno
    ));
    if (!empty($log) && empty($log['status'])) {
