...
$goods          = pdo_fetch("SELECT * FROM " . tablename('ycs_fxshop_goods') . " WHERE id = :id limit 1", array(
    ':id' => $goodsid
));
...
if ($_W['isajax']) {
    ...
    if (!empty($goods['hasoption'])) {
        ...
        foreach ($allspecs as &$s) {
            $items      = pdo_fetchall("select * from " . tablename('ycs_fxshop_goods_spec_item') . " where  `show`=1 and specid=:specid order by displayorder asc", array(
                ":specid" => $s['id']
            ));
    ...
    $specs  = $allspecs;
    $params = pdo_fetchall("SELECT * FROM " . tablename('ycs_fxshop_goods_param') . " WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc", array(
        ':uniacid' => $uniacid,
        ":goodsid" => $goods['id']
    ));
    $fcount = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_member_favorite') . ' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid and deleted=0 ', array(
        ':uniacid' => $uniacid,
        ':openid' => $openid,
        ':goodsid' => $goods['id']
    ));
    pdo_query('update ' . tablename('ycs_fxshop_goods') . " set viewcount=viewcount+1 where id=:id and uniacid='{$uniacid}' ", array(
        ":id" => $goodsid
    ));
    $history = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_member_history') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1', array(
        ':goodsid' => $goodsid,
        ':uniacid' => $uniacid,
        ':openid' => $openid
    ));
    if ($history <= 0) {
        ...
        pdo_insert('ycs_fxshop_member_history', $history);
    }
    ...
    $ret        = array(
        ...
        'level' => $level,
        'shop' => $shop,
        'goodscount' => pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_goods') . ' where uniacid=:uniacid and status=1 and deleted=0 ', array(
            ':uniacid' => $uniacid
        )),
        'cartcount' => pdo_fetchcolumn('select sum(total) from ' . tablename('ycs_fxshop_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', array(
            ':uniacid' => $uniacid,
            ':openid' => $openid
        )),
        'isfavorite' => $fcount > 0,
