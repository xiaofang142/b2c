...
function m($name = '')
{
    ...
    $model = YCS_FXSHOP_CORE . "model/" . strtolower($name) . '.php';
    if (!is_file($model)) {
        die(' Model ' . $name . ' Not Found!');
    }
    require $model;
    $class_name      = 'Ycs_Dshop_' . ucfirst($name);
...
function p($name = '')
{
    if ($name != 'perm' && !IN_MOBILE) {
        static $_perm_model;
        if (!$_perm_model) {
            $perm_model_file = YCS_FXSHOP_PLUGIN . 'perm/model.php';
    ...
    $model = YCS_FXSHOP_PLUGIN . strtolower($name) . '/model.php';
    if (!is_file($model)) {
        return false;
    }
    require $model;
...
{
    global $_W;
    $set  = pdo_fetch('select sets from ' . tablename('ycs_fxshop_sysset') . ' order by id asc limit 1');
...
function shop_template_compile($from, $to, $inmodule = false)
{
    ...
    if (MWX_FAMILY == 'x' && !preg_match('/(footer|header|account\/welcome|login|register)+/', $from)) {
        $content = str_replace('月琴', '系统', $content);
    }
