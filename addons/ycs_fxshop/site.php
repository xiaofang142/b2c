...
require YCS_FXSHOP_INC.'core.php';
require YCS_FXSHOP_INC.'plugin/plugin.php';
require YCS_FXSHOP_INC.'plugin/plugin_model.php';
class Ycs_fxshopModuleSite extends Core { 
    ...
    public function doWebPlugin(){   
        global $_W,$_GPC;
        require_once YCS_FXSHOP_INC."plugin/plugin.php";
        $plugins = m('plugin')->getAll(); 
        $p = $_GPC['p']; 
        $file = YCS_FXSHOP_PLUGIN.$p."/web.php";
        if(!is_file($file)){ 
    ...
    public function doMobilePlugin(){ 
        ...
        $plugins = m('plugin')->getAll();
        $p = $_GPC['p'];
        $file = YCS_FXSHOP_PLUGIN.$p."/mobile.php";
