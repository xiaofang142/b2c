...
if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $sql       = 'SELECT COUNT(*) FROM ' . tablename('ycs_fxshop_creditshop_log') . " log where 1 {$condition}";
        $total     = pdo_fetchcolumn($sql, $params);
        $list      = array();
        if (!empty($total)) {
            $sql  = 'SELECT log.id,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money FROM ' . tablename('ycs_fxshop_creditshop_log') . ' log ' . ' left join ' . tablename('ycs_fxshop_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
        ...
        $id  = intval($_GPC['id']);
        $log = pdo_fetch('select * from ' . tablename('ycs_fxshop_creditshop_log') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(
            ':id' => $id,
            ':openid' => $openid,
            ':uniacid' => $uniacid
        ));
