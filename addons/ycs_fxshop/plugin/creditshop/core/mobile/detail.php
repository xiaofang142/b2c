...
if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $needpay = false;
        if ($goods['money'] > 0) {
            pdo_delete('ycs_fxshop_creditshop_log', array(
                'goodsid' => $id,
                'openid' => $openid,
                'status' => 0,
                'paystatus' => 0
            ...
            $needpay = true;
            $lastlog = pdo_fetch('select * from ' . tablename('ycs_fxshop_creditshop_log') . ' where goodsid=:goodsid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(
                ':goodsid' => $id,
                ':openid' => $openid,
                ':uniacid' => $uniacid
            ));
        ...
        pdo_insert('ycs_fxshop_creditshop_log', $log);
        $logid = pdo_insertid();
        if ($needpay) {
            $useweixin = true;
        ...
        pdo_update('ycs_fxshop_creditshop_log', $update, array(
            'id' => $logid
        ));
        if ($status == 2) {
