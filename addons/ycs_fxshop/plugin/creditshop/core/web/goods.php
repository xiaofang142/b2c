...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('ycs_fxshop_creditshop_goods', array(
    ...
    $sql      = 'SELECT * FROM ' . tablename('ycs_fxshop_creditshop_goods') . " where  1 and {$condition} ORDER BY displayorder DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
    $list     = pdo_fetchall($sql, $params);
    $total    = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ycs_fxshop_creditshop_goods') . " where 1 and {$condition}", $params);
    $pager    = pagination($total, $pindex, $psize);
    $category = pdo_fetchall("select id,name,thumb from " . tablename('ycs_fxshop_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(
        ':uniacid' => $_W['uniacid']
    ), 'id');
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ycs_fxshop_creditshop_goods', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
            plog('creditshop.goods.edit', "编辑积分商城商品 ID: {$id} <br/>商品名称: {$data['title']}");
        } else {
            pdo_insert('ycs_fxshop_creditshop_goods', $data);
            $id = pdo_insertid();
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('ycs_fxshop_creditshop_goods') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    if ($item['showlevels'] != '') {
    ...
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,title FROM " . tablename('ycs_fxshop_creditshop_goods') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (empty($item)) {
