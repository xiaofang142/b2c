...
if ($op == 'display') {
    if ($_W['isajax']) {
        ...
        $advs        = pdo_fetchall("select id,advname,link,thumb from " . tablename('ycs_fxshop_adv') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(
            ':uniacid' => $uniacid
        ));
    ...
    if ($_W['isajax']) {
        if ($_W['ispost']) {
            ...
            if (empty($shop['id'])) {
                pdo_insert('ycs_fxshop_commission_shop', $shopdata);
            } else {
                pdo_update('ycs_fxshop_commission_shop', $shopdata, array(
                    'id' => $shop['id']
                ));
            }
            show_json(1);
    ...
    if ($_W['isajax']) {
        ...
        if (!empty($shop['selectgoods'])) {
            $goodsids = explode(',', $shop['goodsids']);
            if (!empty($goodsids)) {
                $goods = pdo_fetchall('select id,title,marketprice,thumb from ' . tablename('ycs_fxshop_goods') . ' where uniacid=:uniacid and id in ( ' . trim($shop['goodsids']) . ')', array(
                    ':uniacid' => $_W['uniacid']
                ));
