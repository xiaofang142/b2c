...
if (!class_exists('CommissionModel')) {
    class CommissionModel extends PluginModel
    {
        ...
        public function calculate($orderid = 0, $update = true)
        {
            ...
            if ($set['level'] > 0) {
                foreach ($goods as &$cinfo) {
                    ...
                    if ($update) {
                        pdo_update('ycs_fxshop_order_goods', array(
        ...
        public function getInfo($openid, $options = null)
        {
            ...
            if ($level >= 1) {
                if (in_array('ordercount0', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=0 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid  limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('ordercount', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=1 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid  limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('ordercount3', $options)) {
                    $level1_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=3 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid  limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('total', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . " where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid", array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('ok', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . " where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and ({$time} - o.createtime > {$day_times}) and og.status1=0  and o.uniacid=:uniacid", array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                    foreach ($level1_commissions as $c) {
                ...
                if (in_array('lock', $options)) {
                    $level1_commissions1 = pdo_fetchall('select og.commission1  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . " where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and ({$time} - o.createtime <= {$day_times})  and og.status1=0  and o.uniacid=:uniacid", array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('apply', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . " where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid", array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('check', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . " where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid ", array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                    foreach ($level1_commissions2 as $c) {
                ...
                if (in_array('pay', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . " where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid ", array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                    foreach ($level1_commissions2 as $c) {
                ...
                $level1_agentids = pdo_fetchall('select id from ' . tablename('ycs_fxshop_member') . ' where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid ', array(
                    ':uniacid' => $_W['uniacid'],
                    ':agentid' => $member['id']
                ), 'id');
            ...
            if ($level >= 2) {
                if ($level1 > 0) {
                    if (in_array('ordercount0', $options)) {
                        $level2_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=0 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('ordercount3', $options)) {
                        $level2_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('total', $options)) {
                        $level2_commissions = pdo_fetchall('select commission2  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('ok', $options)) {
                        $level2_commissions = pdo_fetchall('select commission2  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ")  and ({$time} - o.createtime > {$day_times}) and o.status>=3 and og.status2=0 and og.nocommission=0  and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level2_commissions as $c) {
                            $commission = iunserializer($c['commission2']);
                    ...
                    if (in_array('lock', $options)) {
                        $level2_commissions1 = pdo_fetchall('select commission2  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ")  and ({$time} - o.createtime <= {$day_times}) and og.status2=0 and o.status>=3 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('apply', $options)) {
                        $level2_commissions2 = pdo_fetchall('select commission2  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ")  and o.status>=3 and og.status2=1 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('check', $options)) {
                        $level2_commissions3 = pdo_fetchall('select commission2  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ")  and o.status>=3 and og.status2=2 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('pay', $options)) {
                        $level2_commissions3 = pdo_fetchall('select commission2  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ")  and o.status>=3 and og.status2=3 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level2_commissions3 as $c) {
                            $commission = iunserializer($c['commission2']);
                    ...
                    $level2_agentids = pdo_fetchall('select id from ' . tablename('ycs_fxshop_member') . ' where agentid in( ' . implode(',', array_keys($level1_agentids)) . ') and isagent=1 and status=1 and uniacid=:uniacid', array(
                        ':uniacid' => $_W['uniacid']
                    ), 'id');
            ...
            if ($level >= 3) {
                if ($level2 > 0) {
                    if (in_array('ordercount0', $options)) {
                        $level3_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=0 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('ordercount3', $options)) {
                        $level3_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('ycs_fxshop_order') . ' o ' . ' left join  ' . tablename('ycs_fxshop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('total', $options)) {
                        $level3_commissions = pdo_fetchall('select commission3  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('ok', $options)) {
                        $level3_commissions = pdo_fetchall('select commission3  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ")  and ({$time} - o.createtime > {$day_times}) and o.status>=3 and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level3_commissions as $c) {
                            $commission = iunserializer($c['commission3']);
                    ...
                    if (in_array('lock', $options)) {
                        $level3_commissions1 = pdo_fetchall('select commission3  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ")  and o.status>=3 and ({$time} - o.createtime > {$day_times}) and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('apply', $options)) {
                        $level3_commissions2 = pdo_fetchall('select commission3  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ")  and o.status>=3 and og.status3=1 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('check', $options)) {
                        $level3_commissions3 = pdo_fetchall('select commission3  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ")  and o.status>=3 and og.status3=2 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('pay', $options)) {
                        $level3_commissions3 = pdo_fetchall('select commission3  from ' . tablename('ycs_fxshop_order_goods') . ' og ' . ' left join  ' . tablename('ycs_fxshop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ")  and o.status>=3 and og.status3=3 and og.nocommission=0 and o.uniacid=:uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level3_commissions3 as $c) {
                            $commission = iunserializer($c['commission3']);
                    ...
                    $level3_agentids = pdo_fetchall('select id from ' . tablename('ycs_fxshop_member') . ' where uniacid=:uniacid and agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and isagent=1 and status=1', array(
                        ':uniacid' => $_W['uniacid']
                    ), 'id');
        ...
        public function getAgents($orderid = 0)
        {
            global $_W, $_GPC;
            $agents = array();
            $order  = pdo_fetch('select id,agentid,openid from ' . tablename('ycs_fxshop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(
                ':id' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($order)) {
        ...
        {
            global $_W;
            $path = IA_ROOT . "/addons/ycs_fxshop/data/qrcode/" . $_W['uniacid'];
            if (!is_dir($path)) {
                load()->func('file');
                mkdirs($path);
            }
            $url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ycs_fxshop&do=plugin&p=commission&method=myshop&mid=' . $mid;
            if (!empty($posterid)) {
            ...
            return $_W['siteroot'] . 'addons/ycs_fxshop/data/qrcode/' . $_W['uniacid'] . '/' . $file;
        }
        ...
        public function createGoodsImage($goods, $shop_set)
        {
            ...
            $path = IA_ROOT . "/addons/ycs_fxshop/data/poster/" . $_W['uniacid'] . '/';
            if (!is_dir($path)) {
                load()->func('file');
                mkdirs($path);
            }
            ...
            if (!is_file($path . $file)) {
                set_time_limit(0);
                $font   = IA_ROOT . "/addons/ycs_fxshop/static/fonts/msyh.ttf";
                $target = imagecreatetruecolor(640, 1225);
                $bg     = imagecreatefromjpeg(IA_ROOT . '/addons/ycs_fxshop/plugin/commission/images/poster.jpg');
            ...
            return $_W['siteroot'] . "addons/ycs_fxshop/data/poster/" . $_W['uniacid'] . "/" . $file;
        }
        ...
        public function checkAgent()
        {
            ...
            if ($parent_is_agent) {
                if ($parent['openid'] != $openid) {
                    $clickcount = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_commission_clickcount') . ' where uniacid=:uniacid and openid=:openid and from_openid=:from_openid limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':openid' => $openid,
                    ...
                    if ($clickcount <= 0) {
                        ...
                        pdo_insert('ycs_fxshop_commission_clickcount', $click);
            ...
            if ($type == 0) {
                $first = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_member') . ' where id<:id and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid'],
                    ':id' => $member['id']
                ));
                if ($first <= 0) {
        ...
        public function checkOrderConfirm($orderid = '0')
        {
            ...
            $order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime from ' . tablename('ycs_fxshop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(
                ':id' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($order)) {
        ...
        public function checkOrderPay($orderid = '0')
        {
            ...
            $order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime from ' . tablename('ycs_fxshop_order') . ' where id=:id and status>=1 and uniacid=:uniacid limit 1', array(
                ':id' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($order)) {
            ...
            if (!$isagent && empty($set['become_order'])) {
                ...
                if ($set['become'] == 2 || $set['become'] == 3) {
                    ...
                    $can = false;
                    if ($set['become'] == '2') {
                        $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_order') . ' where openid=:openid and status>=1 and uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $openid
                        ));
                        $can        = $ordercount >= intval($set['become_ordercount']);
                    } else if ($set['become'] == '3') {
                        $moneycount = pdo_fetchcolumn('select sum(og.realprice) from ' . tablename('ycs_fxshop_order_goods') . ' og left join ' . tablename('ycs_fxshop_order') . ' o on og.orderid=o.id  where o.openid=:openid and o.status>=1 and o.uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $openid
                        ));
        ...
        public function checkOrderFinish($orderid = '')
        {
            ...
            $order = pdo_fetch('select id,openid, ordersn,goodsprice,agentid,finishtime from ' . tablename('ycs_fxshop_order') . ' where id=:id and status>=3 and uniacid=:uniacid limit 1', array(
                ':id' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($order)) {
            ...
            if (!$isagent && $set['become_order'] == 1) {
                if ($set['become'] == 2 || $set['become'] == 3) {
                    ...
                    $can = false;
                    if ($set['become'] == '2') {
                        $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('ycs_fxshop_order') . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $openid
                        ));
                        $can        = $ordercount >= intval($set['become_ordercount']);
                    } else if ($set['become'] == '3') {
                        $moneycount = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('ycs_fxshop_order') . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1', array(
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $openid
                        ));
                        $can        = $moneycount >= floatval($set['become_moneycount']);
        ...
        function getShop($m)
        {
            ...
            $shop   = pdo_fetch('select * from ' . tablename('ycs_fxshop_commission_shop') . ' where uniacid=:uniacid and mid=:mid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':mid' => $member['id']
            ));
        ...
        function getLevels($all = true)
        {
            global $_W;
            if ($all) {
                return pdo_fetchall('select * from ' . tablename('ycs_fxshop_commission_level') . ' where uniacid=:uniacid order by commission1 asc', array(
                    ':uniacid' => $_W['uniacid']
                ));
            } else {
                return pdo_fetchall('select * from ' . tablename('ycs_fxshop_commission_level') . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc', array(
                    ':uniacid' => $_W['uniacid']
                ));
            }
        }
        function getLevel($openid)
        {
            ...
            $level = pdo_fetch('select * from ' . tablename('ycs_fxshop_commission_level') . ' where uniacid=:uniacid and id=:id limit 1', array(
                ':uniacid' => $_W['uniacid'],
        ...
        function upgradeLevel($openid)
        {
            ...
            foreach ($agents as $agent) {
                ...
                $newlevel        = pdo_fetch('select * from ' . tablename('ycs_fxshop_commission_level') . " where uniacid=:uniacid " . " and {$ordermoney} >= ordermoney and ordermoney>0  order by ordermoney desc limit 1", array(
                    ':uniacid' => $_W['uniacid']
                ));
                if (empty($newlevel)) {
                    $newlevel = pdo_fetch('select * from ' . tablename('ycs_fxshop_commission_level') . " where uniacid=:uniacid " . " and {$commissionmoney} >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1", array(
                        ':uniacid' => $_W['uniacid']
                    ));
                }
