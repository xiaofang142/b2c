...
require YCS_FXSHOP_INC . 'plugin/plugin_processor.php';
class PosterProcessor extends PluginProcessor
{
    ...
    private function responseText($obj)
    {
        ...
        $resp = ihttp_post($_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ycs_fxshop&do=plugin&p=poster&method=build', array(
    ...
    private function responseScan($obj)
    {
        ...
        $poster = pdo_fetch('select * from ' . tablename('ycs_fxshop_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($poster)) {
            return $this->responseDefault($obj);
        ...
        if (empty($url)) {
            if ($qrmember['isagent'] == 1 && $qrmember['status'] == 1) {
                $url = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=ycs_fxshop&do=plugin&p=commission&method=myshop&mid=" . $qrmember['id'];
            } else {
                $url = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=ycs_fxshop&do=shop&mid=" . $qrmember['id'];
            }
        }
        $news = array(
            array(
    ...
    private function responseSubscribe($obj)
    {
        ...
        $qrmember = m('member')->getMember($qr['openid']);
        $log      = pdo_fetch('select * from ' . tablename('ycs_fxshop_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(
            ':openid' => $openid,
            ':posterid' => $poster['id'],
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($log) && $openid != $qr['openid']) {
            ...
            pdo_insert('ycs_fxshop_poster_log', $log);
            $log['id']     = pdo_insertid();
