...
if (!class_exists('PermModel')) {
    class PermModel extends PluginModel
    {
        ...
        private function check($permtype = '')
        {
            ...
            $user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,u.perms as userperms,r.perms as roleperms from ' . tablename('ycs_fxshop_perm_user') . ' u ' . ' left join ' . tablename('ycs_fxshop_perm_role') . ' r on u.roleid = r.id ' . ' where uid=:uid limit 1 ', array(
                ':uid' => $uid
            ));
        ...
        function check_plugin($pluginname = '')
        {
            ...
            $type = intval(@file_get_contents(IA_ROOT . "/addons/ycs_fxshop/data/perm/set"));
            if (!empty($type)) {
                $acid    = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE `uniacid`=:uniacid LIMIT 1", array(
                    ':uniacid' => $_W['uniacid']
                ));
                $ac_perm = pdo_fetch('select  plugins from ' . tablename('ycs_fxshop_perm_plugin') . ' where acid=:acid limit 1', array(
                    ':acid' => $acid
                ));
                if (!empty($ac_perm)) {
                    ...
                    $user_perm = pdo_fetch('select  plugins from ' . tablename('ycs_fxshop_perm_plugin') . ' where uid=:uid limit 1', array(
                        ':uid' => $_W['uid']
                    ));
                    if (!empty($user_perm)) {
        ...
        public function log($type = '', $op = '')
        {
            ...
            pdo_insert('ycs_fxshop_perm_log', $log);
        }
