...
if (!class_exists('TaobaoModel')) {
    class TaobaoModel extends PluginModel
    {
        ...
        {
            global $_W;
            $g = pdo_fetch("select * from " . tablename('ycs_fxshop_goods') . " where uniacid=:uniacid and taobaoid=:taobaoid limit 1", array(
                ':uniacid' => $_W['uniacid'],
                ":taobaoid" => $itemid
            ));
            if ($g) {
		...
		/*增加productprice（原价） start */
		//判断原价是否存在,若没有默认等于从淘宝价格区间最高一部分（待定）；
		if($itemInfoModel1['priceUnits'][1]['price']){
		$item['productprice'] = $itemInfoModel1['priceUnits'][1]['price'];	
		}else{
			if($itemInfoModel1['priceUnits'][0]['rangePrice']){
			$item['productprice'] = $itemInfoModel1['priceUnits'][0]['rangePrice'];	
			}
		}
		/*增加productprice（原价） end */
		/*增加市场价marketprice，start*/
		if($itemInfoModel1['priceUnits'][0]['price']){
			$item['marketprice'] = $itemInfoModel1['priceUnits'][0]['price'];
		}
		/*增加市场价marketprice，end*/
                    ...
                    foreach ($skus as $key => $val) {
                        ...
                        foreach ($options as &$o) {
                            if ($o['skuId'] == $sku_id) {
								...
								$o['marketprice'] =$val['priceUnits'][0]['price'];/*得到marketprice字段*/
								$o['productprice'] =$val['priceUnits'][1]['price'];/*得到productprice字段*/
        ...
        function save_goods($item = array(), $taobaourl = '')
        {
            ...
            $data      = array(
				...
				"productprice" => $item['productprice'],/*增加productprice字段*/
                "pcate" => $item['pcate'],
                "ccate" => $item['ccate'],
                "tcate" => $item['tcate'],
                "sales" => $item['sales'],
            ...
            $goods             = pdo_fetch("select * from " . tablename('ycs_fxshop_goods') . " where  taobaoid=:taobaoid and uniacid=:uniacid", array(
                ":taobaoid" => $item['itemId'],
                ":uniacid" => $_W['uniacid']
            ));
            if (empty($goods)) {
            ...
            $goods_params = pdo_fetchall("select * from " . tablename('ycs_fxshop_goods_param') . " where goodsid=:goodsid ", array(
                ":goodsid" => $goodsid
            ));
            $params       = $item['params'];
            $paramids     = array();
            $displayorder = 0;
            foreach ($params as $p) {
                $oldp    = pdo_fetch("select * from " . tablename('ycs_fxshop_goods_param') . " where goodsid=:goodsid and title=:title limit 1", array(
                    ":goodsid" => $goodsid,
                    ":title" => $p['title']
                ));
                $paramid = 0;
                ...
                if (empty($oldp)) {
                    pdo_insert("ycs_fxshop_goods_param", $d);
                    $paramid = pdo_insertid();
                } else {
                    pdo_update('ycs_fxshop_goods_param', $d, array(
                        "id" => $oldp['id']
                    ));
                    $paramid = $oldp['id'];
                }
            ...
            if (count($paramids) > 0) {
                pdo_query("delete from " . tablename('ycs_fxshop_goods_param') . " where goodsid=:goodsid and id not in (" . implode(",", $paramids) . ")", array(
                    ":goodsid" => $goodsid
                ));
            } else {
                pdo_query('delete from ' . tablename('ycs_fxshop_goods_param') . ' where goodsid=:goodsid ', array(
                    ':goodsid' => $goodsid
                ));
            }
            $specs        = $item['specs'];
            ...
            foreach ($specs as $spec) {
                $oldspec = pdo_fetch("select * from " . tablename('ycs_fxshop_goods_spec') . " where goodsid=:goodsid and propId=:propId limit 1", array(
                    ":goodsid" => $goodsid,
                    ":propId" => $spec['propId']
                ));
                $specid  = 0;
                ...
                if (empty($oldspec)) {
                    pdo_insert("ycs_fxshop_goods_spec", $d_spec);
                    $specid = pdo_insertid();
                } else {
                    pdo_update('ycs_fxshop_goods_spec', $d_spec, array(
                        "id" => $oldspec['id']
                    ));
                    $specid = $oldspec['id'];
                }
                ...
                foreach ($spec_items as $spec_item) {
                    ...
                    $oldspecitem  = pdo_fetch("select * from " . tablename('ycs_fxshop_goods_spec_item') . " where specid=:specid and valueId=:valueId limit 1", array(
                        ":specid" => $specid,
                        ":valueId" => $spec_item['valueId']
                    ));
                    $spec_item_id = 0;
                ...
                if (count($spec_itemids) > 0) {
                    pdo_query("delete from " . tablename('ycs_fxshop_goods_spec_item') . " where specid=:specid and id not in (" . implode(",", $spec_itemids) . ")", array(
                        ":specid" => $specid
                    ));
                } else {
                    pdo_query('delete from ' . tablename('ycs_fxshop_goods_spec_item') . ' where specid=:specid ', array(
                        ':specid' => $specid
                    ));
                }
            ...
            if (count($specids) > 0) {
                pdo_query("delete from " . tablename('ycs_fxshop_goods_spec') . " where goodsid=:goodsid and id not in (" . implode(",", $specids) . ")", array(
                    ":goodsid" => $goodsid
                ));
            } else {
                pdo_query('delete from ' . tablename('ycs_fxshop_goods_spec') . ' where goodsid=:goodsid ', array(
                    ':goodsid' => $goodsid
                ));
            }
            $minprice = 0;
            ...
            foreach ($options as $o) {
                ...
                $do       = array(
					...
					"productprice" => $o['productprice'],/*增加productprice字段*/
                    "skuId" => $o['skuId']
                );
                if ($minprice > $o['marketprice']) {
                    $minprice = $o['marketprice'];
                }
                $oldoption = pdo_fetch("select * from " . tablename('ycs_fxshop_goods_option') . " where goodsid=:goodsid and skuId=:skuId limit 1", array(
                    ":goodsid" => $goodsid,
                    ":skuId" => $o['skuId']
                ));
                $option_id = 0;
                if (empty($oldoption)) {
                    pdo_insert("ycs_fxshop_goods_option", $do);
                    $option_id = pdo_insertid();
                } else {
                    pdo_update('ycs_fxshop_goods_option', $do, array(
                        "id" => $oldoption['id']
                    ));
            ...
            if (count($optionids) > 0) {
                pdo_query("delete from " . tablename('ycs_fxshop_goods_option') . " where goodsid=:goodsid and id not in (" . implode(",", $optionids) . ")", array(
                    ":goodsid" => $goodsid
                ));
            } else {
                pdo_query('delete from ' . tablename('ycs_fxshop_goods_option') . ' where goodsid=:goodsid ', array(
                    ':goodsid' => $goodsid
                ));
            }
            ...
            pdo_update('ycs_fxshop_goods', $d, array(
                "id" => $goodsid
            ));
            return array(
                'result' => '1',
        ...
        function saveToLocal($url)
        {
            ...
            $path  = "images/ycs_fxshop/" . $_W['uniacid'] . "/" . date('Y') . "/" . date('m') . "/";
            load()->func('file');
