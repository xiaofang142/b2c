...
class TaobaoWeb extends Plugin
{
	...
	{
		global $_W, $_GPC;
		$category = pdo_fetchall("SELECT * FROM " . tablename('ycs_dshop_category') . " WHERE uniacid=:uniacid  ORDER BY parentid ASC, displayorder DESC", array(':uniacid' => $_W['uniacid']), 'id');
		if (!empty($category)) {
			$children = '';
