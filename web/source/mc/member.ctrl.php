...
if($do == 'display') {
	...
	$sql = "SELECT uid, uniacid, groupid, realname, nickname, email, mobile, credit1, credit2, credit6, createtime,avatar  FROM ".tablename('mc_members')." WHERE uniacid = '{$_W['uniacid']}' ".$condition." ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
	...
	if(!empty($list)) {
		...
		foreach($list as &$li) {
			...
			if(empty($li['email']) || (!empty($li['email']) && substr($li['email'], -6) == 'ccc.bz' && strlen($li['email']) == 39)) {
...
if($do == 'post') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($_GPC)) {
			...
			if(!empty($_GPC['fanid'])) {
					...
					$_GPC['email'] = md5($_GPC['openid']) . '@ccc.bz';
	...
	if(!empty($profile)) {
		...
		if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == 'ccc.bz' && strlen($profile['email']) == 39)) {
