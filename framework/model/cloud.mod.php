...
function cloud_prepare() {
	...
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		...
		return error('-1', "您的程序需要在三思云云服务平台注册你的站点资料, 来接入云平台服务后才能使用相应功能.");
...
function cloud_m_prepare($name) {
	...
	$dat = cloud_request('http://v2.addons.weixin.mx/gateway.php', $pars);
...
function _cloud_build_params() {
	...
	$pars['family'] = MWX_FAMILY;

	$pars['version'] = MWX_VERSION;

	$pars['release'] = MWX_RELEASE_DATE;
...
function cloud_build() {
	...
	if(!is_error($ret)) {
		...
		if($ret['family'] == 'x' && MWX_FAMILY == 'v') {

			load()->model('setting');

			setting_upgrade_version('x', MWX_VERSION, MWX_RELEASE_DATE);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://v2.addons.weixin.mx/gateway.php', $pars, $headers, 300);
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		...
		return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止，请联系三思云客服！');
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://v2.addons.weixin.mx/gateway.php', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'http://v2.addons.weixin.mx/web/index.php?c=auth&a=passwort&__auth=' . $query;
